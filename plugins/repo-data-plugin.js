  ...
  return {
    ...
    async loadContent() {
      ...
      const shouldFetch =
        ...
        (process.env.CI === "true" &&
          process.env.GITHUB_EVENT_NAME === "schedule") || // Scheduled CI
      ...
      if (!shouldFetch) {
        ...
        try {
          if (fs.existsSync(cacheFile)) {
            console.log(
              "Using cached repository data (add --fetch-repos to update)",
            );
            ...
            console.log(
              `Loaded ${filteredRepos.length} repositories from cache (generated at ${cachedData.generated_at})`,
            );
          ...
          console.warn(
            "Failed to read cached data, falling back to fresh fetch:",
            error.message,
          );
        }
      }
      ...
      try {
        ...
        console.log(
          `After filtering: ${filteredRepos.length} repositories (${data.items.length - filteredRepos.length} omitted)`,
        );
