...
To do so, simply create a `.talon-list` file with a more specific [context header](../TalonScript/context-header.md) than the default. (For example, `lang: en` or `os: mac` main). Talon ensures that the most specific header (your override file) wins.
