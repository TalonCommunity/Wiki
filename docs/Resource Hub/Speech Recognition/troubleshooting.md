...
When Talon is running, it will be in one of a few different [modes](../../Basic%20Usage/basic_usage). One of these modes is a `sleep` mode, which will put Talon in a dormant state where it is listening but not responding to commands. While asleep, Talon will only respond to a minimal set of commands, such as `wake up`, which will put Talon out of sleep and enable Talon to respond to commands.
...
2.  Speak a few different commands. Each utterance is saved to a separate FLAC file in the recordings folder inside the Talon home folder (Talon menu > Scripting > Open ~/.talon).
